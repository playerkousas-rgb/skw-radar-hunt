  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .place-hint {
      ...
      border: 1px solid rgba(0,240,255,0.3); z-index: 1000;
      font-family: -apple-system, BlinkMacSystemFont, sans-serif;
      pointer-events: none; white-space: nowrap;
    }
    ` : ''}
    #searchBox {
      position: fixed; top: ${leaderMode ? '40px' : '10px'}; left: 10px; right: 10px;
      z-index: 1001;
    }
    #searchInput {
      ...
      color: ${darkMode ? '#F1F5F9' : '#333'}; font-size: 13px;
      font-family: -apple-system, BlinkMacSystemFont, sans-serif;
    ...
    .search-item {
      padding: 8px 12px; cursor: pointer; border-bottom: 1px solid ${darkMode ? '#1E293B' : '#eee'};
      font-size: 12px; color: ${darkMode ? '#F1F5F9' : '#333'};
      font-family: -apple-system, BlinkMacSystemFont, sans-serif;
    }
    .search-item:hover { background: ${darkMode ? '#1A2236' : '#f5f5f5'}; }
    .search-item small { color: ${darkMode ? '#64748B' : '#999'}; display: block; margin-top: 2px; font-size: 10px; }
  </style>
</head>
<body>
  ${leaderMode ? '<div class="crosshair"></div><div class="crosshair-dot"></div><div class="place-hint">Tap map to place checkpoint</div>' : ''}
  <div id="searchBox">
  ...
  <script>
    ...
    markers.forEach(function(m) {
      ...
      var popupHTML = '<div class="cp-popup">' +
        ...
        (m.found ? '<div class="cp-found">\\u2713 Found!</div>' : '') +
        (m.hint && !m.found ? '<div class="cp-meta" style="margin-top:3px;font-style:italic">\\ud83d\\udca1 ' + m.hint + '</div>' : '') +
        (m.hasContent ? '<div class="cp-content-hint">\\ud83d\\udcc4 Has content</div>' : '') +
        '</div>';
    ...
    // Coordinate badge on mouse move
  ...
  return (
    <View style={[styles.container, { height: height as any }]}>
      <View style={styles.nativeFallback}>
        <Text style={styles.nativeFallbackIcon}>🗺️</Text>
        <Text style={styles.nativeFallbackText}>Live Map</Text>
        <Text style={styles.nativeFallbackSub}>
          {checkpoints.length} checkpoints • {formatDistance(zoomRange)} range
        </Text>
        <Text style={styles.nativeFallbackCoords}>
...
const styles = StyleSheet.create({
  container: { flex: 1, borderRadius: 14, overflow: 'hidden', backgroundColor: Colors.bgCard },
  loadingOverlay: {
    ...StyleSheet.absoluteFillObject, backgroundColor: Colors.bgCard,
  ...
  nativeFallbackIcon: { fontSize: 48, marginBottom: 12 },
  nativeFallbackText: { color: Colors.text, fontSize: 18, fontWeight: '700' },
  nativeFallbackSub: { color: Colors.textMuted, fontSize: 13, marginTop: 4 },
  nativeFallbackCoords: { color: Colors.primary, fontSize: 12, fontFamily: 'monospace', marginTop: 8 },
});
